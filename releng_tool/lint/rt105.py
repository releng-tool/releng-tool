...
def rt105(state: LintState, pkg: RelengPackage, nodes: list[ast.AST]):
    """
    perform linting "105" -- autotools-only configurations
    ...
    # first, ensure we have no scenario where autotools is the target type
    for node in nodes:
        ...
        for target in node.targets:
            if isinstance(target, ast.Name) and target.id == type_key:
                if isinstance(node.value, ast.Constant):
                    if node.value.value == PackageType.AUTOTOOLS:
                        return

    # confirmed no autotools type -- check for any type-specific configs
    msg = 'unexpected key for non-autotools package'
    key_checks = [
        pkg_key(pkg.name, Rpk.AUTOTOOLS_AUTORECONF),
    ]
